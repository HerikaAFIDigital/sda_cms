...
const sortModules = (modules) => {
  (modules || []).sort(
    (d1, d2) =>
      sortOrder.indexOf(d1.description || "") -
      sortOrder.indexOf(d2.description || "")
  );
  return modules;
};
...
function getModules(langId, moduleKey) {
  ...
  return dbClient
    ...
    .then((ms) => {
      return ms.map((m) => augmentModule(m));
    });
}
...
export function moduleIsEmpty(m) {
  return ["actionCards", "procedures", "videos", "drugs"].reduce(
    (acc, field) => acc && fieldEmpty(m, field),
    true
  );
}
...
export function findWithKey(moduleKey) {
  ...
  const params = [{ name: "@moduleKey", value: moduleKey }];
...
function findUnsorted(moduleKey, langId, role) {
  ...
  return Promise.all([masters, translated]).then((r) => {
    ...
    const mergedModules = ms.map((m) =>
      mergeModule(langId, m, existingModules.get(m.key))
    );
...
export async function update(user, module) {
  ...
  const oldModuleData = module.id
    ? await dbClient.readDoc(module.id)
    : undefined;
  ...
  const toDelete = oldVideoList.filter(
    (oldkey) => newVideoList.indexOf(oldkey) === -1
  );
  const deletePromises = toDelete.map((videoKey) =>
    removeScreen(videoScreenKey(videoKey), module.langId)
  );
  await Promise.all(deletePromises);

  // We look through the existing screen texts to see which videos needs to be added.
  const existingScreenTexts = await dbClient.queryDocs("screens", {
    query: `c.langId = "${module.langId}"`,
  });
  const toAdd = newVideoList.filter(
    (key) =>
      existingScreenTexts.find((e) => e.key === videoScreenKey(key)) ===
      undefined
  );
  const addPromises = toAdd.map((videoKey) =>
    updateScreen(user, [
      {
        key: videoScreenKey(videoKey),
        content: videoKey,
        adapted: videoKey,
        translated: videoKey,
...
export function insert(user, m) {
  ...
  return dbClient
    .upsertDoc(MODULES, stampDoc(module, user))
    .then((clean) => {
    ...
    .then((clean) => {
      return saveMapping("module", clean.key, clean.description).then(
        () => clean
      );
    });
}
