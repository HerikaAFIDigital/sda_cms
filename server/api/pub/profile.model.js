...
/*export async function findProfilesByCountry(country) {
  const globalProfiles = await profileDbClient.queryDocs("appusers", {
    query:
      "c.country = @country AND c.profileModuleScores = @profileModuleScores",
    parameters: [
...
export async function findProfilesByCountry(country) {
  const globalProfiles = await profileDbClient.queryDocs("appusers", {
    projection: "count(1) as 'Count'",
    query: "c.country = @country and c.profileCertificates[0].passed=@passed",
    parameters: [
      { name: "@country", value: country },
...
export async function findUserCountByModuleAndScore(country, targetScore) {
  try {
    const profiles = await profileDbClient.queryDocs("appusers", {
      query: "c.country = @country",
      parameters: [{ name: "@country", value: country }],
    });
    const userCountByModuleScore11 = {};
    profiles.forEach((profile) => {
      const profileModuleScores = profile.profileModuleScores;
      Object.entries(profileModuleScores).forEach(([moduleName, score]) => {
        if (score === targetScore) {
          
          userCountByModuleScore11[moduleName] = (userCountByModuleScore11[moduleName] || 0) + 1;
        }
      });
    });
    console.log("User count for each module with score 11:", userCountByModuleScore11);

    return userCountByModuleScore11;
  } catch (error) {
